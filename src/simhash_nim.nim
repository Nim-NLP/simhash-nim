...
proc buildByFeatures( self:var Simhash, features:seq[tuple[k:string,w:int]]) =
    var 
        v = newSeq[int](self.f)
        h:MD5Digest
        w:int
...
proc initSimhash*(value:string, f=defaultF, reg = defualtReg, hashfunc = toMD5 ) : Simhash =
    result = Simhash(f:f,reg:re(reg),hashfunc:hashfunc)
    result.buildByText(value)

# proc initSimhash(features:seq[tuple[k:string,w:int]] or openArray[tuple[k:string,w:int]], f = defaultF, reg = defualtReg, hashfunc = toMD5 ) : Simhash =
#     result = Simhash(f:f,reg:re(reg),hashfunc:hashfunc)
#     result.build_by_features(features)

# proc initSimhash(features:seq[string] or openArray[string], f = defaultF, reg = defualtReg, hashfunc = toMD5 ) : Simhash =
#     result = Simhash(f:f,reg:re(reg),hashfunc:hashfunc)
#     var 
#         r:seq[tuple[k:string,w:int]] = @[]
#     for x in features:
#         r.add( (k:x,w:1) )
#     result.build_by_features(r)
